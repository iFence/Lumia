...
- **SVG thumbnails in Explorer**: Windows Explorer cannot preview SVG out of the box. After Lumia has run once, its built-in thumbnail provider registers automatically, so `.svg` / `.svgz` files show real thumbnails in Large / Extra large icons views. To manage it manually, run `lumia-app --register-thumbnail-handler` or `lumia-app --unregister-thumbnail-handler`. If a generic icon persists after registering, rebuild the Explorer thumbnail cache: with Explorer closed, delete `%LocalAppData%\Microsoft\Windows\Explorer\thumbcache_*.db`, or simply restart Explorer.
